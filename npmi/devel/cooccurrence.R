...
# @codings Tibble mit den Spalten feature und item (ohne Duplikate)
...
get_cooccurrence <- function(codings,metrics = T, npmi = F,.progress = NULL) {
  ...
  codings.lag0 <- codings %>%
    count_pairs() %>%
    rename(code_source=feature1,code_target=feature2)
  ...
  if (metrics) {
    ...
    codes <- codings %>%
      ...
      mutate(p.msg = n.msg / n_distinct(codings$item))

    codings.lag0 <- codings.lag0 %>%

      # Share of messages with cooccurring code in all messages
      mutate(p = n / n_distinct(codings$item)) %>%
      left_join(select(codes,feature,n_source=n.msg,p_source=p.msg),by=c("code_source"="feature")) %>%
      left_join(select(codes,feature,n_target=n.msg,p_target=p.msg),by=c("code_target"="feature")) %>%
...
# @codings Tibble mit den Spalten feature und item
get_cooccurrence_shuffled <- function(codings,metrics = T,npmi = F,.progress = NULL) {

  #TODO: Beim shufflen keine Duplikate erzeugen
  codings$feature <- sample(codings$feature,length(codings$feature),replace=T)
  codings <- codings %>%
    distinct(feature,item)
...
# - feature (z.B. eine Kategorie),
# - item (z.B. ID eines Kommentars)
# - item_prev (z.B. ID des vorangegangenen Kommentars)
get_sequence <- function(codings,metrics = T, npmi = F,.progress = NULL) {
  ...
  codings <- codings %>%
    distinct(feature,item,item_prev)
  ...
  if (metrics) {
    codes <- codings %>%
      count(feature) %>% rename(n.msg = n) %>%
      mutate(p.msg = n.msg / n_distinct(codings$item))

    # Anzahl maximal möglicher  Sequenzen
    #nmax <-  (n_distinct(codings$item) - n_distinct(codings$diskussion_no))
    nmax <- codings %>%
      filter(!is.na(item_prev)) %>%
      distinct(item) %>%
      nrow()
    ...
    codings.lag1 <- codings.lag1 %>%
      ...
      left_join(select(codes,feature,n_next=n.msg,p_next=p.msg),by=c("code_next"="feature")) %>%
      left_join(select(codes,feature,n_prev=n.msg,p_prev=p.msg),by=c("code_prev"="feature")) %>%
...
# Shuffled die Codings für Resampling, indem die Codes neu zugeteilt werden
# @codings Tibble mit den Spalten feature, item und item_prev
...
# @codings Tibble mit den Spalten feature, item and weight.
...
get_llr <- function(codings) {
  ...
  codings.lag0 <- codings %>%
    pairwise_count(feature,item, diag=F,upper=T) %>%
    rename(code_source=feature1,code_target=feature2)
...
# # @codings Tibble mit den Spalten feature, item und item_prev
# codings.random <- tibble(item = c(1:100000)) %>%
#   mutate(item_prev=lag(item)) %>%
#
#   # In Threads zerschneiden (20% ohne Eltern)
#   mutate(item_prev = ifelse(runif(nrow(.)) < 0.2,NA,item_prev )) %>%
#
...
#   mutate(feature = sample(10,nrow(.),replace=T))  %>%
#
#   distinct(item,item_prev,feature)
#
...
#
# pairwise_count <- function (tbl, feature, item, ...)
# {
#
...
#         mutate(feature1=character(0),feature2=character(0),value=numeric(0))
#     }
#
#     else {
#       ret <- tbl %>% tidyr::nest(..data = c(!! enquo(feature), !! enquo(item))) %>%
#         mutate(..data = purrr::map(..data,pairwise_count,!! enquo(feature),!! enquo(item))) %>%
#         tidyr::unnest(..data)
#     }
#
#     ret <- ret  %>%
...
#   # Empty
#   if (nrow(tbl) == 0) {
#     ret <- tibble(feature1=character(0),feature2=character(0),value=numeric(0))
#     return(ret)
#   }
#
#
...
#   # Distinct
#   tbl <- tbl %>%
#     distinct(!!feature, !!item, .keep_all = TRUE)
#
...
#   ret <- reshape2::melt(as.matrix(mat),varnames = c("feature1", "feature2"), as.is = TRUE) %>%
#     tbl_df() %>%
#     rename(n = value) %>%
